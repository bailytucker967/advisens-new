...
const RESPONSE_QUESTIONS = [
  ...
  { key: "clarifyBeforeAdvice" as const, label: "What I would want to clarify before giving advice", placeholder: "Questions you would ask, information you would need, or assumptions you would want to validate.", required: true },
  { key: "howDecisionsMade" as const, label: "How decisions are typically made in cases like this", placeholder: "How you would typically work through decisions, trade-offs, or key milestones with a client.", required: true },
  { key: "feePhilosophy" as const, label: "Fee philosophy", placeholder: "How you charge, what your fee structure looks like, and how fees are typically structured for similar cases.", required: true },
  { key: "whoThisSuits" as const, label: "Who this approach tends to suit", placeholder: "Client profiles, situations, or preferences that typically align well with your approach.", required: true },
];
...
export default function AdvisorCasePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const clarifyBeforeAdvice = (responseSections.clarifyBeforeAdvice || "").trim();
    const howDecisionsMade = (responseSections.howDecisionsMade || "").trim();
    const feePhilosophy = (responseSections.feePhilosophy || "").trim();
    const whoThisSuits = (responseSections.whoThisSuits || "").trim();
    const allComplete = approach && clarifyBeforeAdvice && howDecisionsMade && feePhilosophy && whoThisSuits;
    if (!allComplete || !guidelinesConfirmed) {
      setError("Please complete all response questions and confirm the guidelines.");
      return;
    }
    setError("");
    setSubmitting(true);
      ...
      <main className="relative z-10 flex-1 px-4 pb-16 pt-8 md:px-8">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-900/40 backdrop-blur-xl p-6 md:p-8">
            ...
            {/* Case submitter - anonymous to advisors (user identity is not revealed) */}
            <section className="mb-8 p-4 rounded-xl border border-white/10 bg-white/5">
              ...
              <p className="text-sm text-slate-300">Anonymous</p>
            </section>
            ...
            {alreadyResponded ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    type="submit"
                    disabled={!responseSections.approach?.trim() || !responseSections.clarifyBeforeAdvice?.trim() || !responseSections.howDecisionsMade?.trim() || !responseSections.feePhilosophy?.trim() || !responseSections.whoThisSuits?.trim() || !guidelinesConfirmed || submitting}
