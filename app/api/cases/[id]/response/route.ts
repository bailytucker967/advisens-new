...
export async function POST(
  ...
  try {
    ...
    // All structured fields are required
    if (hasStructured) {
      const missing = [];
      if (!sections!.clarifyBeforeAdvice) missing.push('What I would want to clarify before giving advice');
      if (!sections!.howDecisionsMade) missing.push('How decisions are typically made');
      if (!sections!.feePhilosophy) missing.push('Fee philosophy');
      if (!sections!.whoThisSuits) missing.push('Who this approach tends to suit');
      if (missing.length > 0) {
        return NextResponse.json(
